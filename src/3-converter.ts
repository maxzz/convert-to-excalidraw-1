...
export async function convertMermaidToExcalidraw(mermaidCode: string, verbose: boolean): Promise<any> {
    const { mermaidBundleUrl, excalidrawBundleUrl, htmlPath } = resolveLoaderPaths();
    ...
    try {
        ...
        // Create the HTML file and wait for it to be created
        ...
        // Execute the files conversion

        const result = await page.evaluate(
            async (code) => {
                try {
                    const { elements, files } = await (window as any).mermaidToExcalidraw.parseMermaidToExcalidraw(code, { fontSize: 20, });
...
function resolveLoaderPaths(): { mermaidBundleUrl: string; excalidrawBundleUrl: string; htmlPath: string; } {
